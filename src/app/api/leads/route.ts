  ...
  try {
    const lead = await prisma.lead.create({
      data: {
        ...
        diagnosisGoal: payload.diagnosis?.goal ?? null,
        diagnosisStage: payload.diagnosis?.stage ?? null,
        diagnosisUrgency: payload.diagnosis?.urgency ?? null,
        diagnosisRecommendation: payload.diagnosis?.recommendedSolution ?? null,
        status: 'new',
      },
      select: { id: true },
    });
